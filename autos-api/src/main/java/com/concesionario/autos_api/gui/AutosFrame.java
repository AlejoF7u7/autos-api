...
public class AutosFrame extends JFrame {
    ...
    // Variable para saber quién está usando la pantalla
    ...
    public AutosFrame(int rol) {
        ...
        iniciarComponentes();
        configurarPermisos(); // Aquí decidimos qué botones mostrar
        cargarDatosTabla();
    }
    ...
    private void iniciarComponentes() {
        ...
        btnComprar.setBounds(550, 120, 200, 30); // Ocupa espacio visible
        btnComprar.setBackground(new Color(241, 196, 15)); // Amarillo
        btnComprar.setForeground(Color.BLACK);
        btnComprar.setFont(new Font("Arial", Font.BOLD, 14));
        btnComprar.setVisible(false); // Oculto por defecto
        add(btnComprar);
        ...
        scroll.setBounds(30, 200, 800, 380);
        add(scroll);
    ...
    private void configurarPermisos() {
        if (rolUsuario == 1) {
            ...
            btnComprar.setVisible(false); // Admin no compra
        } else {
            // SOY CLIENTE
            btnGuardar.setVisible(false);
            btnActualizar.setVisible(false);
            btnEliminar.setVisible(false);
            btnComprar.setVisible(true); // Cliente sí compra

            // Bloqueamos escritura en cajas de texto
    ...
    private void cargarDatosTabla() {
        ...
        for (Auto a : lista) {
            String estado = (a.isDisponible() != null && a.isDisponible()) ? "Disponible" : "VENDIDO";
    ...
    private void guardarAuto() {
        try {
            Auto auto = new Auto(txtPlaca.getText(), txtMarca.getText(), txtModelo.getText(), Double.parseDouble(txtPrecio.getText()));
    ...
    private void actualizarAuto() {
        ...
        try {
            ...
            auto.setDisponible(true); // Si editamos, asumimos que sigue disponible
    ...
    private void comprarAuto() {
        if (txtId.getText().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Selecciona un auto primero");
            return;
        }

        // Verificar si ya está vendido visualmente
        int fila = tablaAutos.getSelectedRow();
        String estadoActual = modeloTabla.getValueAt(fila, 5).toString();
        if(estadoActual.equals("VENDIDO")) {
            JOptionPane.showMessageDialog(this, "¡Este auto ya fue vendido!", "Error", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int confirm = JOptionPane.showConfirmDialog(this, "¿Deseas comprar el " + txtMarca.getText() + " " + txtModelo.getText() + "?");
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                Long id = Long.parseLong(txtId.getText());

                // Creamos un objeto auto con los mismos datos, pero DISPONIBLE = FALSE
                ...
                autoCompra.setDisponible(false);

                if (apiClient.actualizarAuto(id, autoCompra)) {
                    JOptionPane.showMessageDialog(this, "¡Felicidades! Compra realizada con éxito.");
                    limpiarFormulario();
                    cargarDatosTabla(); // La tabla mostrará "VENDIDO"
                }
            } catch (Exception ex) {
